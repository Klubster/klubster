...
insert into auth.users (id, email) values
  ...
  ('0a000000-0000-4000-8000-0000000000a6', 'lecture.a@example.com'),
  ('0b000000-0000-4000-8000-0000000000b1', 'president.b@example.com'),
  ('ff000000-0000-4000-8000-0000000000f1', 'super.admin@example.com');

/**
 * LES CINQ RÔLES D'ÉQUIPE, RÉELLEMENT POSÉS.
 *
 * Jusqu'au 02/08/2026, `tresorier` et `secretaire` ne pouvaient pas être enregistrés :
 * `profiles_role_check` ne connaissait que quatre valeurs, et `lecture` non plus. Les
 * fixtures gardaient ces comptes en `adherent`, faute de mieux.
 *
 * `20260802120000_roles_attribuables.sql` a élargi la contrainte. Les comptes portent
 * donc enfin leur vrai rôle, et `tests/db/20-roles.sql` peut éprouver la matrice ligne
 * par ligne plutôt que constater qu'elle est inapplicable.
 */
insert into public.profiles (id, organisation_id, email, nom, prenom, role) values
  ...
  ('0a000000-0000-4000-8000-0000000000a4', '0a000000-0000-4000-8000-000000000001', 'tresorier.a@example.com',  'A', 'Tresorier', 'tresorier'),
  ('0a000000-0000-4000-8000-0000000000a5', '0a000000-0000-4000-8000-000000000001', 'secretaire.a@example.com', 'A', 'Secretaire','secretaire'),
  ('0a000000-0000-4000-8000-0000000000a6', '0a000000-0000-4000-8000-000000000001', 'lecture.a@example.com',    'A', 'Lecture',   'lecture'),
...
begin
  ...
  raise notice 'Fixtures posées : 2 clubs, 8 comptes (5 rôles d''équipe), 3 adhérents.';
end $$;
